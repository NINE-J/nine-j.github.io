---
id: 019e9b29-fc03-7087-8f04-a7a3827d6d6b
slug: b9f1-devlog-토스-nodejs-developer-사전-과제-readme
publish: false
draft: true
title: 토스 Node.js Developer 사전 과제 README
description: 사전 과제 공개 후 README 파악
author: Nine
date: 2026-06-06T13:21:20
categories:
tags:
  - toss
  - pre-task
...
* 해당 과제를 진행함에 있어, AI Tool 을 사용 하는 것은 자율입니다.
* AI Tool 을 이용 시, 사용 기록 원본을 HISTORY.md, 복수파일이면 HISOTRY/\*에 넣어 주세요. (하단 예시 참고)
  * cursor : 우측 상단 - Export Chat
  * claude-code : /export (슬래시 커맨드 사용)
* **AI Tool 사용 시, 기록이 첨부되어 있지 않으면 감점 사유가 될 수 있습니다.**
* AI 활용 방식에 대해 간단히 설명하는 파일을 함께 제출해 주세요. (선택)
...
* 본 과제 템플릿은 Node.js 24버전에서 작성되었습니다. node 24 이상 버전을 사용하길 권장드립니다.
* "npm install"
* "npm run start:data-source" 를 하면, 데이터 소스 서버가 구동됩니다.
...
* 10분 주기로 각 _**포트별 api와 csv에서 Transaction 정보를 수집하고**_ 이를 이용하여 MergeTransaction을 구성하고, database에 저장해 주세요.
...
* 데이터 소스 설명
  * [http://localhost의](http://localhost의) 4001,4002,4003번 포트는 Transaction 데이터를 조회할 수 있는 api를 제공하고 있습니다.
  ...
  * csv파일에서도 Transaction 정보가 적재되어 있으며, 이는 [data-source/transaction.csv](./data-source/transaction.csv)에 주기적으로 파일이 업로드 된다고 가정합니다.
    ...
    * csv파일에 대한 설명은 다음 링크를 참고해 주세요. [https://ko.wikipedia.org/wiki/CSV\_(파일\_형식)](https://ko.wikipedia.org/wiki/CSV_\(파일_형식\))
  * [http://localhost의](http://localhost의) 4596번 포트는 StoreTransaction 데이터를 조회할 수 있는 api를 제공하고 있습니다.
...
* node\_modules,dist 폴더를 삭제하시고 해당 프로젝트를 압축해 주세요.
* 압축된 파일이름은 다음과 같은 예시대로 작성해 주세요.
  * 홍길동\_20230101\_Nodejs.zip
