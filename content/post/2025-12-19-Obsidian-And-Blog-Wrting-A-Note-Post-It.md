...
![](/static/assets/images/Pasted%20image%2020251219044301.png)
...
- `![](/static/assets/images/location/image.png)`
Markdown 문법이 기대하는 이미지 첨부는 이런 형태다.
- `![alt](/static/assets/images/location/image.png)`
      ...
      - name: Filter and Transform Posts
        run: |
          ...
          for file in $PUBLISH_FILES; do
            ...
            # 위키링크 치환: ![](/static/assets/images/image.png) -> ![](/static/assets/images/image.png)
            perl -i -pe 's/!\[\[(?!https?:\/\/)(.*?)\]\]/![](/static/assets/images/\/static\/assets\/images\/$1)/g' "$dest"

            # 표준 마크다운 경로 보정: ![](/static/assets/images/) -> ![](/static/assets/images/...)
            perl -i -pe 's/!\[(.*?)\]\((?!https?:\/\/|\/assets\/|\/static\/assets\/images\/)(.*?)\)/!\[$1\](\/static\/assets\/images\/$2)/g' "$dest"
          done
...
- **Regular Expression (Regex)**: 정규표현식. 특정 패턴의 텍스트를 찾고 바꾸는 규칙이다. 위키링크(`![](/static/assets/images/)`)를 마크다운 링크(`![](/static/assets/images/)`)로 변환하는 등에 핵심적으로 사용된다.
